...
> 把 VS Code 裡 Claude Code 的英文介面換成中文。**繁體中文（台灣）** 與 **簡體中文（中國大陸）** 各一套完整翻譯，自動跟隨系統語言，也可以隨時手動切換。

Claude Code 的側邊欄、對話框、按鈕、通知原本全是英文。安裝這個擴充功能後，它們都會變成中文——不是機器轉換的簡繁互換，而是兩套各自人工翻譯、逐條校對的版本：

- **繁體中文（台灣）**：台灣慣用的軟體用語，例如「外掛程式」「資料夾」「對話階段」「延伸思考」。
- **簡體中文（中國大陸）**：大陸常用的软件术语，例如「插件」「文件夹」「会话」「扩展思考」。

目前共 **1461 條**翻譯規則（繁簡各一套），涵蓋設定、權限、勾點、沙箱、記憶、外掛程式、用量統計等所有對話框。
