...
- 🌏 **雙語內建**：繁體中文（台灣）與簡體中文（中國大陸）各一套完整翻譯，各 1461 條。
- 🧭 **自動偵測 / 手動切換**：預設「跟隨系統」依 VS Code 顯示語言自動選擇；也可在選單或設定中手動指定繁體或簡體。
- 🖥️ **介面隨語言切換**：連本擴充自身的狀態列、選單、通知都會依所選語言顯示繁體或簡體；指令標題與設定說明則走 VS Code 原生 NLS（`package.nls.*.json`），依 VS Code 顯示語言呈現單一語言，不會繁簡並列。
- 🈶 **一鍵重載介面**：Claude 的側邊欄與編輯器分頁標題列各多一顆 🌏 按鈕，按下即「重新套用 + 就地重載介面」。Claude 的 webview 都設了 `retainContextWhenHidden`，載入後不會重建——這正是「關掉 VS Code 重開，對話還在卻變回英文」的原因（視窗還原時 webview 比本擴充功能早一步載入）。啟動時若真的套用了翻譯，會主動問一次要不要重載；狀態列也會顯示 **待重載介面**。
...
| `claudeCodeZh.autoReloadClaudeUi` | `false` | 套用後直接就地重載介面，不再詢問（會重載視窗內全部 webview） |
...
webview/index.js.bak（原始英文，基底）
        │  前置規則 → 內建規則（依語言：zh-TW.json / zh-CN.json，各 1461 條）→ 後置規則
        ▼
